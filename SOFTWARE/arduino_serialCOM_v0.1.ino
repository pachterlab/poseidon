...
#define X_ACCEL 1000.0 // X steps per second per second
#define Y_ACCEL 1000.0 // Y
#define Z_ACCEL 1000.0 // Z
...
void executeThisFunction() {
  if (strcmp(mode, "SETTING") == 0) {
    ...
    if (strcmp(setting, "ONE") == 0){
      jogOne();
    }
    else if (strcmp(setting, "FEW") == 0){
      jogFew();
    }
...
//===================================
// These two functions are similar to runOne() and runFew in fact we might be able 
// to just call runOne() when we want to jog one, runFew() when we want to jog few
// and runAll() when we want to jog all
void jogOne() {
  // make the optional argument a string which tells us ONE, FEW, ALL
  // realistically we could combine these three functions into one with the proper conditions
  // Lets write out the runAll() function then try to combine them.
    runOne();
}
...
void jogAll() {
  ...
  if (strcmp(dir, "B") == 0) {
    stepper1.move(-jog1Delta);
    stepper2.move(-jog2Delta);
    stepper3.move(-jog3Delta);
    while (stepper1.distanceToGo() <= 0 && stepper2.distanceToGo() <= 0 && stepper3.distanceToGo() <= 0) {
      stepper1.runSpeedToPosition();
      stepper2.runSpeedToPosition();
      stepper3.runSpeedToPosition();
